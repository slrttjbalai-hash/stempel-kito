...
export default function HelpTab() {
  ...
  const appScriptCode = `/**
 * STEMPEL KITO Kota Tanjungbalai - Google Sheets Integration Script (OPTIMIZED DUAL-SHEET VERSION)
 * 
...
function setupSheet() {
  ...
  if ((sheetRec && sheetRec.getLastRow() > 1) || 
      (sheetArch && sheetArch.getLastRow() > 1) || 
      (sheetFac && sheetFac.getLastRow() > 1)) {
    hasData = true;
  }
  ...
  if (hasData) {
    ...
    clearData = true; // No data exists, so safe to initialize fully
  }
  ...
  // 1. Setup Laporan Sheet (Aktif / Belum Dikunjungi)
  if (!sheetRec) {
  ...
  // 1b. Setup Archive Sheet (Sudah Dikunjungi)
  if (!sheetArch) {
    sheetArch = ss.insertSheet(SHEET_ARCHIVE);
  }
  if (clearData) {
    sheetArch.clear();
    sheetArch.getRange(1, 1, 1, headersRec.length).setValues([headersRec]);
  } else {
    sheetArch.getRange(1, 1, 1, headersRec.length).setValues([headersRec]);
  }
  const headerArchRange = sheetArch.getRange(1, 1, 1, headersRec.length);
  headerArchRange.setBackground("#0284c7") // Sky-600 (Biru untuk Arsip Laporan Selesai)
                .setFontColor("#ffffff")
                .setFontWeight("bold")
                .setFontFamily("Inter")
                .setFontSize(10)
                .setHorizontalAlignment("center")
                .setVerticalAlignment("middle");
  sheetArch.setRowHeight(1, 30);
  sheetArch.setFrozenRows(1);
  sheetArch.getRange(1, 1, 1000, headersRec.length).setFontFamily("Inter").setFontSize(9);

  for (let i = 1; i <= headersRec.length; i++) {
    sheetArch.autoResizeColumn(i);
    const width = sheetArch.getColumnWidth(i);
    if (width < 130) {
      sheetArch.setColumnWidth(i, 130);
    }
  }

  // 2. Setup Facilitators Sheet
  setupFacilitatorSheet(clearData);

  if (clearData) {
    SpreadsheetApp.getUi().alert("✅ Sukses memformat bersih seluruh tabel Laporan, Arsip Kunjungan & Daftar Petugas!");
  } else {
    SpreadsheetApp.getUi().alert("✅ Sukses merapikan tampilan & struktur tabel! Data lama Anda tetap AMAN dan terjaga.");
  }
}

function setupFacilitatorSheet(clearData) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheetFac = ss.getSheetByName(SHEET_FACILITATORS);
  if (!sheetFac) {
  ...
  if (clearData) {
    ...
    // Pre-fill default facilitators if completely empty/cleared
...
function onOpen() {
  ...
  ui.createMenu("🟢 Menu STEMPEL KITO")
    ...
    .addSeparator()
    .addItem("🧹 Pindahkan Data Sudah Dikunjungi ke Arsip", "moveVisitedToArchive")
    .addItem("Saring & Ekspor Laporan Terpadu", "filterAndExportIntegrated")
    .addItem("Rapikan Baris & Format Status", "autofitAndStyleRows")
    .addToUi();
}

// Fungsi Memindahkan data 'Sudah Dikunjungi' ke sheet Arsip secara massal
function moveVisitedToArchive() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheetRec = ss.getSheetByName(SHEET_RECORDS);
  let sheetArch = ss.getSheetByName(SHEET_ARCHIVE);
  ...
  if (!sheetArch) {
    ...
    const headersRec = [
      ...
      "Foto KK/KTP (Base64)", "Foto Depan Rumah (Base64)", "Foto Kontrol Kunjungan (Base64)"
    ];
    sheetArch.getRange(1, 1, 1, headersRec.length).setValues([headersRec]);
    sheetArch.getRange(1, 1, 1, headersRec.length).setBackground("#0284c7").setFontColor("#ffffff").setFontWeight("bold");
    sheetArch.setFrozenRows(1);
  }
  
  const lastRow = sheetRec.getLastRow();
  if (lastRow <= 1) {
    SpreadsheetApp.getUi().alert("ℹ️ Tidak ada data untuk dipindahkan di sheet utama.");
    return;
  }
  ...
  // Memproses dari baris paling bawah agar tidak merusak urutan index saat baris dihapus
  for (let i = values.length - 1; i >= 0; i--) {
    const row = values[i];
    const statusKunjungan = String(row[18]).trim();
    
    if (statusKunjungan === "Sudah Dikunjungi" || statusKunjungan === "Terverifikasi") {
      const targetId = row[0];
      let archFoundRow = -1;
      const lastRowArch = sheetArch.getLastRow();
      
      if (lastRowArch > 1) {
        const archIds = sheetArch.getRange(2, 1, lastRowArch - 1, 1).getValues();
        for (let j = 0; j < archIds.length; j++) {
          if (String(archIds[j][0]).trim() === String(targetId).trim()) {
            archFoundRow = j + 2;
            break;
          }
        }
      ...
      // Simpan/perbarui ke sheet arsip
      if (archFoundRow !== -1) {
        sheetArch.getRange(archFoundRow, 1, 1, row.length).setValues([row]);
      } else {
        sheetArch.appendRow(row);
      }
      
      // Hapus dari sheet utama (aktif)
      sheetRec.deleteRow(i + 2);
      movedCount++;
    }
  }
  ...
  SpreadsheetApp.getUi().alert("✅ Sukses! Berhasil mengarsipkan " + movedCount + " data 'Sudah Dikunjungi' ke sheet '" + SHEET_ARCHIVE + "'. Kini pengiriman data Anda akan kembali kencang dan responsif!");
}
    ...
    if (lastRowArch > 1) {
      ...
      for (let i = 0; i < valuesArch.length; i++) {
        ...
        records.push({
          ...
          jenisPengaduan: String(row[16]),
          jenisLayanan: String(row[17]),
          statusKunjungan: row[18] ? String(row[18]) : "Sudah Dikunjungi",
          tanggalPemeriksaan: String(row[19]) || "-",
          catatanPemeriksa: String(row[20]) || "-",
          diinputOleh: row[21] ? String(row[21]) : "Admin",
          namaPendata: row[22] ? String(row[22]) : "",
...
// Menghapus laporan rujukan secara permanen dari Sheet (mencari di kedua Sheet Laporan Aktif & Arsip)
function deleteRecord(id) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheets = [ss.getSheetByName(SHEET_RECORDS), ss.getSheetByName(SHEET_ARCHIVE)];
  let deleted = false;
  ...
  if (deleted) {
    return { status: "success", message: "Laporan " + id + " berhasil dihapus secara global" };
  }
...
function syncRecord(data) {
  ...
  const targetId = data.id || data.clientId || data.client_id || "rec-" + new Date().getTime();
  const statusKunjungan = data.statusKunjungan || "Belum Dikunjungi";
  const isVisited = (statusKunjungan === "Sudah Dikunjungi" || statusKunjungan === "Terverifikasi");
  
  // Tentukan lembar target penyimpanan dan lembar pembersihan
  const targetSheet = isVisited ? sheetArch : sheetRec;
  const cleanupSheet = isVisited ? sheetRec : sheetArch;
  ...
  if (foundRowTarget !== -1) {
    targetSheet.getRange(foundRowTarget, 1, 1, rowData.length).setValues([rowData]);
  } else {
    targetSheet.appendRow(rowData);
  }
  
  // 2. Bersihkan/Hapus dari Lembar Lainnya (Menghindari Duplikasi Data)
  const lastRowCleanup = cleanupSheet.getLastRow();
  if (lastRowCleanup > 1) {
    const valuesCleanup = cleanupSheet.getRange(2, 1, lastRowCleanup - 1, 1).getValues();
    for (let i = 0; i < valuesCleanup.length; i++) {
      if (String(valuesCleanup[i][0]).trim() === String(targetId).trim()) {
        cleanupSheet.deleteRow(i + 2);
        break;
      }
    }
  }
